...
- `pnpm run build:public-isolation` 必须临时注入唯一非公开候选，以 `lstat`、打开后身份复核和分块字节扫描检查 `dist` 内所有普通文件及独立 `public/search-index.json`，并在任何成功或失败路径恢复审核队列的原始字节；非普通部署项、队列 JSON 非数组或损坏都必须 fail closed 且不得回显原文，构建子进程必须先完成有界进程树终止再恢复队列或向调用方拒绝。
