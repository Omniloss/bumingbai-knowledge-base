...
- `pnpm run build:public-isolation` 与审核队列写入共享有界跨进程排他锁；隔离门必须在锁内验证 `root/data/review/queue` 链、用同目录临时文件和重验后的 rename 注入及精确恢复原字节。它必须以 `lstat`、打开后身份复核和分块字节扫描检查 `dist` 内所有普通文件及独立 `public/search-index.json`，并在任何成功或失败路径恢复审核队列的原始字节；非普通部署项、队列 JSON 非数组或损坏都必须 fail closed 且不得回显原文，构建子进程必须先完成有界进程树终止再恢复队列或向调用方拒绝。
- Windows 构建进程树必须由 `@vscode/windows-process-tree` 在构建期间连续采样，并以 PID 创建时间复核后终止；不得退回到构建退出或超时后才用 WMI 重建父子关系。该原生包只使用预编译二进制且禁止运行安装脚本，构建成功、失败和超时都必须清理仍存活的已跟踪后代，跟踪器在清理异常时也必须停止。
