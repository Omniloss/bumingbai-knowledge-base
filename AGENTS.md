...
- `pnpm run build:public-isolation` 必须临时注入唯一非公开候选，按字节顺序扫描 `dist` 内所有普通文件及独立 `public/search-index.json`，并在任何成功或失败路径恢复审核队列的原始字节；非普通部署项、队列 JSON 非数组或损坏都必须 fail closed 且不得回显原文，构建子进程须有界终止并清理进程树。
