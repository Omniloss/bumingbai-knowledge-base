...
- 同步产生的候选推荐必须写入 `data/review/sync-candidates.json`，保留原始行、来源 URL、官方快照 `retrievedAt` 和稳定 DOM 定位；只接受四个精确栏目名，段落开头的 `strong`/`b` 标题可带一个末尾中英文冒号。解析必须限制不可信 HTML 的长度、节点和深度，审核队列不得被公共目录或搜索构建读取。
- `src/sync/run-sync.ts` 的审核队列发布必须用唯一同目录临时文件、独占创建、进程内按目标串行化和 rename；父目录与目标都要以 `lstat`/`realpath` 拒绝符号链接或真实路径逃逸，失败不得破坏既有队列。
