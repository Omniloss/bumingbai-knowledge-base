...
- `--focus` 只用于交互反馈，不作为装饰。
- 禁用和加载色在当前静态外壳中不适用。筛选空态与搜索空态或错误态沿用 `--muted` 文字和结构规则线，不新增会把状态含义只交给颜色的专用色。
...
展示标题使用均衡换行并优先在标点和空白边界换行，只有整段确实超宽时才允许字内回退，避免中文姓名或单字被拆开成为孤行。`CjkTitle` 把冒号、逗号、句号、问号、叹号、分号、顿号和体貌助词“了”附着到前一词，禁止标点或“了”成为新行首；详情标题把目录中已核验的嘉宾姓名作为受保护短语传入，不依赖通用分词器猜测姓名边界。固定量词、宾语和修饰短语使用 `keep-together` 行内原语保持语义完整；Task 1 覆盖“下一部”“值得读或看的作品”和“通过核验的数据”，Task 6 还覆盖公开推荐、官方身份、纠错流程和数据来源页面中的关键短语与品牌名。详情 h1 与证据节目标题使用同一策略；375px 下已确认的中文姓名必须保持在同一行。
...
| `--card-min` | `16rem` | 详情封面列和卡片内容的可读宽度下限 |
...
- 资料网格在 `60rem` 及以上固定三列，在 `40rem` 到 `60rem` 之间固定两列，在 `40rem` 以下固定一列；卡片不设固定高度。
...
| Record grid | 明确响应列数的文章集合；首页近期区块和索引页共用 | `>=60rem` 三列、`40rem–60rem` 两列、`<40rem` 一列，列与行均遵循 4px 网格 | 交由卡片链接处理 | 交由卡片链接处理 | 交由卡片链接处理 | 不适用 | 构建时生成，不显示骨架 | 集合为空时由 Empty state 接管 | 不适用 |
...
| Episode recommendation group | `section > h3 + recommendation records`；确认推荐人或中性“推荐记录”变体 | 每条保留官方原文，并以 `WorkCard` 链接公开作品；非公开记录不进入 DOM | 交由作品链接处理 | 交由作品链接处理 | 交由作品链接处理 | 不适用 | 构建时生成 | 无公开推荐时显示通用空态；存在非公开资料时只显示中性非披露通知，不出现候选、核验、审核、审查、评审或英文 review/candidate 词汇 | 不适用 |
...
| Filter bar | 始终可见的原生 GET form，两个有标签的 select、结果计数和 reset | 纸面控件、方角细边框；查询参数固定为 `media` 和 `status`；选项只列出当前公开集合可命中的媒介和状态，不显示待核验、已排除或当前无记录的值 | select 和按钮边框变为海军蓝 | reset 使用焦点红反馈 | 控件显示统一焦点轮廓，键盘顺序为媒介、状态、重置 | 不适用，筛选条件都可选 | 不适用，操作同步完成 | 无匹配结果时显示 Empty state，完整列表仍保留在 DOM | 无运行时请求，不定义错误态；`40rem` 以下表单保持可见并纵向堆叠，不引入抽屉 |
...
| SearchBox | `visible label + input[type=search] + persistent div[role=listbox] + sibling live message`；首页统一检索 | 命名 listbox 始终作为 `aria-controls` 指向的结果容器存在；两个字符后下载公开静态索引，方角输入框和规则线结果组不形成浮层 | 输入框边框变为海军蓝，结果链接显示下划线 | 结果链接使用焦点红 | 输入框和结果链接显示统一焦点轮廓，方向键保留输入焦点并更新活动结果 | 不适用，空查询保留空 listbox 和搜索说明 | 首次索引请求期间不伪造加载 UI | 无匹配时保持 listbox 为空，在相邻 live message 显示简短说明，避免把非 option 内容放进 listbox | 请求或解析静态索引失败时清空候选结果，在相邻 live message 显示“搜索暂不可用，请稍后重试。”；不暴露候选或技术细节 |
| Entity collection | `section > header + record list`；人物和主题详情页复用 | 顶部细规则线、方角、无容器底色；仅连接公开实体和可公开推荐证据 | 内部链接使用海军蓝与下划线 | 内部链接使用焦点红 | 链接显示统一焦点轮廓 | 不适用 | 构建时生成 | 没有合格记录时明确标示“暂无已核验记录” | 不适用 |

只实现静态目录确实存在的 default、hover、active、focus 和筛选 empty。页面在构建时读取目录，不添加 loading 或 error 横幅；原生链接和筛选控件没有虚构的 disabled 状态。

## 6. Motion & Interaction

页面不使用入场动画。链接和控件的 hover、active 与 focus 反馈立即发生，不移动布局。作品筛选只用一个框架无关的客户端脚本切换现有卡片的 `hidden` 状态，并以 `history.replaceState` 同步查询参数，不请求运行时数据。跳转链接依靠原生键盘焦点出现。`prefers-reduced-motion: reduce` 下强制关闭平滑滚动，并把任何后续非必要动画和过渡时长压到 `0.01ms`、迭代次数限制为一次，防止新组件绕过减弱动态偏好。
...
- 首页 SearchBox 在输入少于两个字符时不请求索引也不显示结果；输入达到两个字符后只请求 `/search-index.json`。先按标题或别名前缀排序，再按查询词元交集排序，按实体类型分组为普通链接。Escape 清空查询与结果；ArrowDown、ArrowUp 更新活动结果；Enter 跳转到活动结果。请求或解析失败时不保留候选、不创建 option，并显示诚实的不可用说明。
