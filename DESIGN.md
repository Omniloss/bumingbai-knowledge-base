...
| SearchBox | `label + input[type=search] + grouped listbox`；首页统一检索 | 两个字符后下载公开静态索引；方角输入框和规则线结果组，不形成浮层 | 输入框边框变为海军蓝，结果链接显示下划线 | 结果链接使用焦点红 | 输入框和结果链接显示统一焦点轮廓，方向键保留输入焦点并更新活动结果 | 不适用，空查询只显示搜索说明 | 首次索引请求期间不伪造加载 UI | 无匹配时显示简短的无结果说明 | 请求失败时不显示虚构错误或候选内容 |
...
- 首页 SearchBox 在输入少于两个字符时不请求索引也不显示结果；输入达到两个字符后只请求 `/search-index.json`。先按标题或别名前缀排序，再按查询词元交集排序，按实体类型分组为普通链接。Escape 清空查询与结果；ArrowDown、ArrowUp 更新活动结果；Enter 跳转到活动结果。
