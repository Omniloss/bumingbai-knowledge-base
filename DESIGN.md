...
| SearchBox | `label + input[type=search] + grouped listbox`；首页统一检索 | 两个字符后下载公开静态索引；方角输入框和规则线结果组，不形成浮层 | 输入框边框变为海军蓝，结果链接显示下划线 | 结果链接使用焦点红 | 输入框和结果链接显示统一焦点轮廓，方向键保留输入焦点并更新活动结果 | 不适用，空查询只显示搜索说明 | 首次索引请求期间不伪造加载 UI | 无匹配时显示简短的无结果说明 | 请求或解析静态索引失败时清空候选结果，并显示“搜索暂不可用，请稍后重试。”；不暴露候选或技术细节 |
